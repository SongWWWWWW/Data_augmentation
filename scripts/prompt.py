def get_prompt(prompt_name:str):
    ...
    I will present you with an instance where the model provided incorrect responses. The classification labels are defined as follows:
    {{"FAVOR": 1, "NONE": 2, "AGAINST": 0}}.
    ...
    {generate_num} verification data points, each enclosed in <verification_i>, where i is the sequential number of the verification set.
    Each <verification_i> should contain the following:

       1. <text>: A multi-sentence passage containing the spurious pattern. Try your best to extend the length of the text to reach the length of the input instance's text as much as possible. .
       2. <target>: An entity mentioned in the text.
       3. <ground_truth>: The true label for the classification task.
    Ensure that the ground truth of the generated data is ascertainably correct; if its correctness cannot be determined, leave the field blank.
