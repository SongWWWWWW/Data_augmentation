...
def get_llm_full_name(llm_name):
    if llm_name == 'claude3-sonnet':
        return 'anthropic.claude-3-sonnet-20240229-v1:0' if os.environ.get(
            'AWS_REGION_NAME') else 'claude-3-sonnet-20240229'
    elif llm_name == 'claude3-haiku':
        return 'anthropic.claude-3-haiku-20240307-v1:0' if os.environ.get(
            'AWS_REGION_NAME') else 'claude-3-haiku-20240307'
    return llm_name
        ...
        try:
            ...
            for e in responses:
                if isinstance(e, ModelResponse):
                    continue
                elif isinstance(e, OpenAIRateLimitError) or isinstance(e, OpenAIAPIError) or isinstance(e,
                                                                                                        OpenAITimeout):
                    exception = e
                    break
                else:
